...
| `MAENG_EffStrainPathline` | 입자 궤적 기반 누적 유효 변형률 + trail |
| `MAENG_Strainmap` / `Fwd` / `Bwd` | 전방/후방 pathline 누적 변형률 맵 + streakline |
| `MAENG_Pathlines` / `MAENG_PathlinesArrow` | 입자 경로선 추적 / 화살표 시각화 |
| `MAENG_GridPathline` / `Rotated` / `Stretch` / `MAENG_GridMove` | 그리드 추적 · 변형 시각화 |
| `MAENG_Streaklines` | 연속 주입 유맥선 생성 |
| `MAENG_SSZExtract` / `SSZBandExtract` / `SSZShapeTime` / `SSZResidenceMap` | 2차전단영역(SSZ) 추출·시간 적층·체류시간 |
| `MAENG_FindStagnationPoint` | 프레임별 정체점 탐지 |
| `MAENG_CuttingSpeedAnalyze` | ROI 기반 절삭속도 통계 검증 |
| `MAENG_SpotVelocityTime` | 특정 지점 속도의 시간 추이 |
| `MAENG_VelocityDistrib` | 라인 프로파일 속도 분포 |
...
| `MAENG_ChipBoundaryMask` / `MAENG_ChipMaskPlayback` | 칩 경계 마스크 생성/재생 확인 |
| `MAENG_MaterialMaskCheck` | 재료 vs 배경 마스크 판별 검증 뷰어 |
| `MAENG_ViewRawImages` | 원본 이미지 폴더 재생 |
