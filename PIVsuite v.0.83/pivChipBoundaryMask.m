function chipMask = pivChipBoundaryMask(pivData)
% pivChipBoundaryMask — discontinuous 칩의 "프레임별 칩/void 경계 마스크" 다중단서 검출
% ----------------------------------------------------------------------------
%  목적: 매 프레임 이동하는 칩-비칩 경계(갭/void)를 자동 검출하여
%        pivStrainmapFwd의 frame_wall에 OR로 넣을 수 있는 마스크 chipMask 생성.
...
%
%  좌표: 픽셀 (영상/PIV 격자 기준)
%  실행: 인자 없이 F5  또는  chipMask = pivChipBoundaryMask(pivData)
...
% ----------------------------------------------------------------------------
  ...
  if strcmp(q,'예')
    ...
    fprintf(['\n>> chipMask 생성 완료: %d×%d×%d.  base 작업공간 + chipMask.mat 저장.\n', ...
             '------------------------------------------------------------------\n', ...
             ' pivStrainmapFwd 의 frame_wall 함수에 아래 블록을 추가하세요\n', ...
...
%% ============================================================
%% 로컬: NaN-aware 구배 (벽 경계 편측차분) — pivStrainmapFwd와 동일
%% ============================================================
