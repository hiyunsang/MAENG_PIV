function [cutStat] = cuttingSpeedAnalyze(pivData, varargin)
% cuttingSpeedAnalyze - 직교절삭(Orthogonal Cutting) PIV 결과에서
...
if nargin < 1 || isempty(pivData)
    try
        ...
        fprintf('[cuttingSpeedAnalyze] base 워크스페이스의 ''pivData''를 자동으로 불러왔습니다.\n');
    catch
        error('cuttingSpeedAnalyze:noPivData', ...
            ['워크스페이스에 ''pivData'' 변수가 없습니다.\n', ...
             '   먼저 마스크 스크립트를 실행하여 pivData를 생성한 뒤\n', ...
             '   이 함수를 실행하거나, 명령창에서 cuttingSpeedAnalyze(pivData) 형태로 호출하세요.']);
    end
end
...
if ~all(isfield(pivData, {'U','V','X','Y'}))
    error('cuttingSpeedAnalyze:invalidInput', ...
...
if isempty(opt.roi)
    ...
    yMax = yMaxAll;
    fprintf('[cuttingSpeedAnalyze] ROI 자동 설정 (좌측 1/2 x 하단 1/2).\n');
else
...
nPtsROI = nnz(inROI);
if nPtsROI == 0
    error('cuttingSpeedAnalyze:emptyROI', ...
...
for kt = 1:Nt
    ...
    switch lower(FILTER_MODE)
        ...
        otherwise
            error('cuttingSpeedAnalyze:badFilterMode', ...
...
if ~any(validF)
    error('cuttingSpeedAnalyze:noHorizontalVecs', ...
...
if nargout == 0
    ...
    fprintf('[cuttingSpeedAnalyze] 결과 구조체를 base 워크스페이스 ''cutStat''에 저장했습니다.\n\n');
    clear cutStat;
end
