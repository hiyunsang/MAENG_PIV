function sszData = pivSSZExtract(pivData, varargin)
% pivSSZExtract — 단일 프레임에서 SSZ(이차전단대) 고전단 밴드 추출
% --------------------------------------------------------------------------
...
% ★ 실행법 2 (오버레이 단계 호출):
%     ssz = pivSSZExtract(pivData,'Frame',120,'ROI',[x1 x2 y1 y2],'Show',false);
%
% 출력 sszData:
...
if nargin < 1 || isempty(pivData)
    try
        ...
        fprintf('[pivSSZExtract] base 워크스페이스의 pivData 사용\n');
    catch
...
if isempty(ROI_RECT)
    ...
    fprintf('[pivSSZExtract] ROI = [%.1f %.1f %.1f %.1f] px\n', ROI_RECT);
end
...
fprintf(['[pivSSZExtract] frame=%d  signal=%s  ROI유효=%.0f%%  thr=%.3f  ' ...
...
%% ========================================================================
%% 보조 1: NaN-인지 편측 미분 (raw 모드 fallback; pivEffStrain 규약)
%% ========================================================================
