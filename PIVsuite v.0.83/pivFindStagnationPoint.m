function stagData = pivFindStagnationPoint(pivData, varargin)
% pivFindStagnationPoint - 직교절삭 PIV 결과에서 프레임별 정체점 검출 (v2)
%
...
%   stagData = pivFindStagnationPoint(pivData, 'MaskImage', maskPath, ...
...
if nargin == 0
    ...
    % 자기 자신을 함수로 재호출
    stagData = pivFindStagnationPoint(pivData, ...
        'MaskImage',     maskFound, ...
        'ROI',           DEFAULT_ROI, ...
...
if opt.Verbose
    fprintf('\n[pivFindStagnationPoint v2] Nt = %d\n', Nt);
end
...
if sum(searchMask(:)) < 3
    error('pivFindStagnationPoint:EmptySearch', ...
...
if ~isempty(opt.MaskImage)
    if ischar(opt.MaskImage) || isstring(opt.MaskImage)
        if exist(char(opt.MaskImage), 'file') == 2
            ...
            warning('pivFindStagnationPoint:MaskFileMissing', ...
...
figure('Name', 'pivFindStagnationPoint v2 — DEBUG', 'Color', 'w');
