%% MAENG_SEQ_MULTICORE - 마스크 시퀀스 PIV 병렬 실행 (one-click parallel runner)
% (멀티코어 병렬 PIV · 마스킹 시퀀스 · 재생 한 번이면 끝)
% MAENG_SEQ와 동일한 계산을 여러 MATLAB 워커로 나눠 빠르게 수행합니다.
% =========================================================================
...
% (b) 함수 폴더: 새로 뜬 워커는 path가 초기화되므로 core/postprocess를 명시적으로 물려줌
rootDir = fileparts(thisScript);
% 워커 부트스트랩: 함수경로 추가 → 작업폴더 이동 → 스크립트 실행
boot = sprintf('addpath(''%s'',''%s'');cd(''%s'');run(''%s'')', ...
               fullfile(rootDir,'core'), fullfile(rootDir,'postprocess'), curDir, thisScript);
