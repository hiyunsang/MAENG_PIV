function eff = MAENG_StrainmapBwd(pivData)
% MAENG_StrainmapBwd — 역방향 "현재 형상" 누적 유효변형률 + 유맥선(Streakline) 오버레이
% ----------------------------------------------------------------------------
%  변형률 (배경 색):
...
%   • Coast Limit(관성 돌파)로 마스크/벽 통과 방지 — MAENG_Streaklines 로직 이식
%
%  좌표: 픽셀(영상 오버레이 기준)
%  실행: 인자 없이 F5  또는  eff = MAENG_StrainmapBwd(pivData)
% ----------------------------------------------------------------------------
...
% MAENG_Streaklines 의 Coast Limit 로직을 시드 방향으로 벡터화.
...
%% ============================================================
%% 로컬: 증분 유효변형률 (von Mises 등가, MAENG_EffStrain 동일식)
%% ============================================================
