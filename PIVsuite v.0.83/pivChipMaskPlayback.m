function pivChipMaskPlayback(pivData)
% pivChipMaskPlayback — 칩/void 경계 마스크를 영상 위에 쭈루룩 재생 (판단용)
% ----------------------------------------------------------------------------
...
%     pivChipBoundaryMask 에 넣고 전체 배치로 chipMask 생성.
%
%  검출 로직은 pivChipBoundaryMask 와 동일(영상+품질+발산+중앙값 융합).
%  실행: 인자 없이 F5  또는  pivChipMaskPlayback(pivData)
% ----------------------------------------------------------------------------

  %% ===== 검출 파라미터 (pivChipBoundaryMask 와 동일하게 유지) =====
  ...
  disp('>> 경계 마스크 재생 완료.  파라미터가 맘에 들면 같은 P 값을 pivChipBoundaryMask 에 넣고 배치하세요.');
end

%% ============================================================
%% 핵심: 단일 프레임 경계 마스크 (pivChipBoundaryMask 와 동일)
%% ============================================================
